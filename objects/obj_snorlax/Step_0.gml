...
if(active and global.turn == TURNS.enemy and global.phase == PHASES.attack and sleep == 0 and speed == 0) {
	...
	if(n){
		move_towards_point(global.current_self.x, global.current_self.y, 10)
	} else if(n == 0) {
		sleep = choose(3, 4, 5)
		max_sleep = sleep
	}
...
if(active) {
	...
	if(!audio_is_playing(sound_red_army) and !audio_is_playing(sound_snorlax) and !audio_is_paused(sound_red_army) and max_sleep != sleep) {
